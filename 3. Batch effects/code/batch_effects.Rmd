...
```{r}
edata <- t(scale(t(edata), scale=FALSE, center=TRUE)) # scale działa na kolumnach więc transponujemy dane żeby skalować geny, które są w wierszach a nie w kolumnach 
svd.out <- svd(edata)
...
```{r}
# spearman, bo nie skalowalismy, uzywa sie go gdy mamy outliery, korelacja rang
...
We use the microarray gene expression data on 57 bladder samples, that were processed in 5 batches [pl: serii]. This dataset is available as a R/Bioconductor package, `bladderbatch`. This dataset is well known to have been confounded. Read the original study:
...
plot(row.variances, row.means, pch=19, main="Mean vs. Variance relationship") # dla kazdego genu: sredni sygnal vs variancja, w eksperymentach szukamy genow roznicujacych, patrzac na variancje redukujemy dane
...
```{r}
pheno.plot = pData(bladderEset) %>% rownames_to_column("series")

colors = tibble(series=pheno.plot$series, cancer=as.character(pheno.plot$cancer))
colors$cancer[colors$cancer == "Cancer"] = "#fec3a6"
colors$cancer[colors$cancer == "Normal"] = "#efe9ae"
colors$cancer[colors$cancer == "Biopsy"] = "#cdeac0"

cancer = colors$series[colors$cancer == "#fec3a6"]
normal = colors$series[colors$cancer == "efe9ae"]
biopsy = colors$series[colors$cancer == "cdeac0"] 

pheno_df = spread(pheno.plot, batch, series)
pheno_df = pheno_df[1:57,4:8]
...
batch_3 =  na.omit(batch_3)
batch_4 = na.omit(batch_4)
batch_5 = na.omit(batch_5)

data <- list("1" = batch_1, "2" = batch_2, "3"=batch_3, "4"=batch_4, "5"=batch_5)
data <- lapply(data, `length<-`, max(lengths(data)))

DF <- as.data.frame(data, col.names = c("Batch_1", "Batch_2", "Batch_3", "Batch_4", "Batch_5"))
DF[is.na(DF)]=''

library(htmlwidgets)
library(dplyr)
library(DT)
x = datatable(DF, rownames = FALSE, options = list(paging = FALSE)) %>%
  formatStyle(columns = 1:5, background = styleEqual(colors$series, colors$cancer)) %>%
  saveWidget(file="./figures/lukasik_problem_1_table.html", selfcontained = FALSE, title="Table showing batch effects")

#legend
color <- c("#fec3a6", "#efe9ae", "#cdeac0")
meaning <- c("Cancer", "Normal", "Biopsy")
legend <- data.frame(color,meaning)
y = datatable(legend, rownames = FALSE, options = list(paging = FALSE)) %>%
  formatStyle('color', backgroundColor = styleEqual(c("#fec3a6", "#efe9ae", "#cdeac0"), c("#fec3a6", "#efe9ae", "#cdeac0"))) %>%
  saveWidget(file="./figures/lukasik_problem_1_legend.html", title="Legend")

```
Continuation of homework:

The problem is that, for example, the first batch contains only cancer cells, the third batch normal, and the fourth biopsy. In the further part of the analysis, we will not know whether we are grouping according to the type of cell or according to the batch, so this effect should be eliminated.
...
```{r}
mod = lm(edata[1,] ~ as.factor(pheno$cancer) + as.factor(pheno$batch)) # budujemy model liniowy y~x1+x2, probujemy wytlumaczyc jaki jest poziom ekstresji genow
# as.factor(pheno$batch)3
# 1.43092
# 
...
library(broom)
mod_tidy <- tidy(mod)
ggplot(mod_tidy) + geom_histogram(aes(x=estimate), bins = 100, fill="darkorange") # plaskie -> intercept, wykres wszystkich wspolczynnikow
...
ggplot(mod_tidy %>% filter(term == "as.factor(pheno$cancer)Cancer")) + geom_histogram(aes(x=estimate), bins = 100, fill="darkorange") # od 0 do 2.5 jest tzw. "ciążki ogon",  czesto jezeli wiemy, ze dana tkanka jest rakowa, ekspresja jest wieksza
...
# na czerwono - rozklady teoretyczne
```
...
```{r}
png("./figures/lukasik_problem_2_bladder.png",height=700,width=700)
...
png("./figures/lukasik_problem_2_bladder_combat.png",height=700,width=700)
heatmap.2(combat_edata[,order(pheno$batch)],
          main = "Bladder Cancer Data Clustered Cleaned by ComBat", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
```{r}
cols <- rep('black', ncol(combat_edata))
cols[colnames(combat_edata) %in% cancer] <- "#fec3a6"
cols[colnames(combat_edata) %in% normal] <- "#efe9ae"
cols[colnames(combat_edata) %in% biopsy] <- "#cdeac0"
png("./figures/lukasik_problem_3.png",height=700,width=700)
heatmap.2(cor(edata, method = "pearson"),
          main = "Pearson correlations statistics of samples", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
# czerwona linia - combat i linear model nachodzą na siebie
```
...
ggsave('./figures/lukasik_problem_4_model1_hist.png')

# different way of looking at coefficients from many different models
# the vertical line indicates the zero coefficient.
ggplot(mod1_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
ggsave('./figures/lukasik_problem_4_model1_scatter.png')
```
...
ggsave('./figures/lukasik_problem_4_model2_hist.png')

# different way of looking at coefficients from many different models
# the vertical line indicates the zero coefficient.
ggplot(mod2_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
ggsave('./figures/lukasik_problem_4_model2_scatter.png')
...
# histogram of estimates of particular regression terms.
ggplot(mod2_tidy %>% filter(term == 'as.factor(bottomly_pheno$strain)DBA/2J')) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
ggsave('./figures/lukasik_problem_5_combat_hist.png')
...
ggplot(mod2_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
ggsave('./figures/lukasik_problem_5_combat_scatter.png')
...
ggsave('./figures/lukasik_problem_5_sva_hist.png')

ggplot(modsva_tidy, aes(estimate, term)) +
     geom_point() +
     geom_vline(xintercept = 0)
ggsave('./figures/lukasik_problem_5_sva_scatter.png')
