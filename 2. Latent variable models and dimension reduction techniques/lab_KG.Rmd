...
```{r}
png("lukasik_problem1.png",height=700,width=700)
heatmap.2(edata,
          main = "Bottomly Clustered", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
          col=my_palette,       # use on color palette defined earlier 
          scale = "col",        # scale in the column direction
          # both rows and columns are clustered by default
...
```{r}
PC_left = data.table(svd.out$u,pData(bottomly.eset))
ggplot(PC_left) + geom_point(aes(x=V1, y=V2, col=as.factor(strain)))
ggsave(
  "lukasik_problem3.png",
  plot = last_plot(),
)
```
...
```{r}
library(cowplot)
plot1 <- ggplot(PC_left) + geom_violin(aes(x=as.factor(strain), y=V1),draw_quantiles = c(0.25, 0.5, 0.75))
plot2 <- ggplot(PC_left) + geom_violin(aes(x=as.factor(strain), y=V2),draw_quantiles = c(0.25, 0.5, 0.75))
plot3 <- ggplot(PC_left) + geom_violin(aes(x=as.factor(strain), y=V3),draw_quantiles = c(0.25, 0.5, 0.75))
plot4 <- ggplot(PC_left) + geom_violin(aes(x=as.factor(strain), y=V4),draw_quantiles = c(0.25, 0.5, 0.75))
plot5 <- ggplot(PC_left) + geom_violin(aes(x=as.factor(strain), y=V5),draw_quantiles = c(0.25, 0.5, 0.75))
plot_grid(
